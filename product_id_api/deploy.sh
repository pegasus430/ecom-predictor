...
EOF
sudo su postgres -c "createdb product_id_api"
...
pip install -r $ROOT_DIR/tmtext/product_id_api/requirements.txt

echo "# Adding Nginx config..."
sudo tee /etc/nginx/sites-available/product_id_api <<EOF
server {
    ...
    location /static {
        alias $ROOT_DIR/tmtext/product_id_api/app/static;
    }
...
sudo ln -s /etc/nginx/sites-available/product_id_api /etc/nginx/sites-enabled/product_id_api
sudo service nginx restart

echo "# Adding Uwsgi config..."

sudo tee $ROOT_DIR/tmtext/product_id_api/app.ini <<EOF
[uwsgi]
...
chdir = $ROOT_DIR/tmtext/product_id_api
virtualenv = $ROOT_DIR/tmtextenv
home = $ROOT_DIR/tmtextenv
wsgi-file = $ROOT_DIR/tmtext/product_id_api/wsgi.py
...
sudo tee /etc/supervisor/conf.d/product_id_api.conf <<EOF
[program:product_id_api]
environment=PATH="$ROOT_DIR/tmtextenv/bin"
directory = $ROOT_DIR/tmtext/product_id_api
user = www-data
command = $ROOT_DIR/tmtextenv/bin/uwsgi app.ini
redirect_stderr=true
stderr_logfile = $ROOT_DIR/logs/product_id_api.err.log
stdout_logfile = $ROOT_DIR/logs/product_id_api.log
