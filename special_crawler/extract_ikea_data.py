...
class IkeaScraper(Scraper):
    ...
    def check_url_format(self):
        ...
        """
        m = re.match(r"^http://www.ikea.com/us/en/.*$", self.product_page_url)
        return not not m
    ...
    def _product_name(self):
        return self.tree_html.xpath('//div[contains(@class, "productName")]/text()')[0].strip()
    ...
    def _description(self):
        short_description = self.tree_html.xpath('//div[@id="custMaterials"]')[0].text_content()
    ...
    def _image_urls(self):
        try:
            data = re.findall(r"var jProductData = (\{.*?\})\;", " ".join(self.tree_html.xpath("//script//text()")), re.DOTALL)
            data = json.loads(data[0])
            image_list = data['product']['items'][1]['images']['normal']
        except:
            return None

        if image_list:
            image_list = ["http://www.ikea.com"+r for r in image_list]
            return image_list
        elif self.tree_html.xpath("//img[@id='productImg']/@src"):
            main_image_url = self.tree_html.xpath("//img[@id='productImg']/@src")
            main_image_url = "http://www.ikea.com" + main_image_url
    ...
    ##########################################
    def _price(self):
        return self.tree_html.xpath("//div[@id='prodPrice']//span[contains(@class,'packagePrice')]/text()")[0].strip()
    ...
    def _categories(self):
        categories = self.tree_html.xpath("//ul[@id='breadCrumbs']/li/a/text()")
    ...
    def _brand(self):
        return self.tree_html.xpath("//span[@itemprop='brand']//span//text()")[0].strip()
