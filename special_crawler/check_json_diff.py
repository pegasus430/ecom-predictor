...
logger = logging.getLogger('check_json_diff')
...
def main():
    p = argparse.ArgumentParser(
        ...
        epilog='NOTE: If there are no regexes in your JSON do not use the '
               ...
               '   ./check_json_diff.py path/to/file1.json path/to/file2.json \n'
               '\n'
               'To compare a single json file against a directory of models:\n'
               '    ./check_json_diff.py --use_model path/to/file.json '
               'path/to/models\n'
               '\n'
               'To compute the diff between to JSON documents: \n'
               '    ./check_json_diff.py -d path/to/new.json path/to/old.json'
