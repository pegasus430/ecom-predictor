...
pip install $ROOT_DIR/tmtext/retail_submission_api/requirements.txt
...
server {
    ...
    location /static {
        alias $ROOT_DIR/tmtext/retail_submission_api/app/static;
    }
...
sudo tee $ROOT_DIR/tmtext/retail_submission_api/app.ini <<EOF
[uwsgi]
...
chdir = $ROOT_DIR/tmtext/retail_submission_api
virtualenv = $ROOT_DIR/tmtextenv
home = $ROOT_DIR/tmtextenv
wsgi-file = $ROOT_DIR/tmtext/retail_submission_api/wsgi.py
...
directory = $ROOT_DIR/tmtext/retail_submission_api
user = www-data
command = $ROOT_DIR/tmtextenv/bin/uwsgi app.ini
redirect_stderr=true
stderr_logfile = $ROOT_DIR/logs/retail_submission_api.err.log
stdout_logfile = $ROOT_DIR/logs/retail_submission_api.log
...
redirect_stderr=true
stderr_logfile = $ROOT_DIR/logs/retail_submission_crawler.err.log
stdout_logfile = $ROOT_DIR/logs/retail_submission_crawler.log
stdout_logfile_maxbytes = 100MB
stdout_logfile_backups=30
autostart = true
autorestart = true
