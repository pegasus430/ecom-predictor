...
def run(input_file, output_file):
    ...
    for pc9, data in input_data.iteritems():
        ...
        if web_id not in products_cache:
            url = 'https://www.macys.com/shop/product/?ID={}'.format(web_id)
            products_cache[web_id] = MacysScraperAux.load_product(url)

        product = products_cache[web_id]

        if product:
            data['color_url'] = MacysScraperAux.get_color_url(product, data['color'])
            data['image_urls'] = MacysScraperAux.get_image_urls(product, data['color'])
            data['video'] = MacysScraperAux.get_video(product)
            data['size_chart'] = MacysScraperAux.get_size_chart(product)
